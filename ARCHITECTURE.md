...
- **ploop × tx — Stop hook 동거.** 두 plugin은 조정 코드 없이 같은 Stop event를 hook한다.
  ...
  최적화이지 무결성 요건이 아니다.
- **refine × tx — 청소도 gate를 지난다**(MEMORY 불변식 1).
...
- **project-unit 격리 — install cache는 execute-only다.** plugin은 세션 스코프(user scope + 현재
  project)와 전역 참(published manifest, Python toolchain)만 읽고, 어디에도 쓰지 않는다 — cache는
  ...
  그 root에 고정해(-P·-B) caller의 env·cwd와 절연한다. 강제는 `tests/test_plugin_runtime.py`.
  의존성이 생기는 미래 plugin은 cache가 아니라 CLAUDE_PLUGIN_DATA에 manifest-diff 패턴으로
  환경을 둔다.
