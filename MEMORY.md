...
**이 문서는 기억 시스템의 설계 정본이다**: 무엇이 기억되고, 어디에 있고, 어떻게 검증되며,
...
- **정책 층은 어느 길이든 자작해야 한다.** 업스트림 프롬프트는 질문을 상시 게이트로
  만든다(ploop launch는 질문을 상시 게이트로 두지 않는다 — 정면 충돌). 방치된 루프에서 미드턴
...
- **외부 규약이라는 사실 자체가 drift 방어다.** automata의 플러그인은 에이전트가
...
- **fork하지 않는다** — fork는 validate 유지보수를 떠안아 채택의 이유를 소멸시키고, 기억 문법을
  ...
  보장한다.
- **exit plan** — 포맷은 플레인 텍스트라 기억은 도구와 독립이다. 소멸 시에는 같은 seam
  뒤에서 사용 커맨드를 재구현하거나 MIT 포크한다.
...
추적 텍스트는 어디서든 gitignored·미추적(시스템 temp 포함) 경로를 지시하지 않는다 — 자기완결은
조사 기록만의 의무가 아니고, CI docs-form-check가 전 추적 `.md`에서 형식 검사한다.

### 자리 규약

**설계 정본** — 소유 단위(레포 루트 / 필요를 입증한 하위 단위 / 자기 불변식을 가진 횡단
도메인)당 하나, 고정 이름 — 구조 단위는 `ARCHITECTURE.md`(storefront 의무 단위는 README 겸용),
횡단 도메인은 도메인 이름 — 이름이 회상 키다. 허브는 위성에 링크 위임하고 어느 방향도
...
**용어** — 소유 정본의 `## 용어` 섹션, **용어당 home 하나**(단위 용어는 단위 정본, 횡단 용어는
루트 정본). 산문이 정의를 이미 운반하면 섹션을 만들지 않는다. 형식: `**term** — 1~2문장 정의 +
referent`. 용어는 사실이 아니라 규약이다 — 참의 조건이 측정이 아니라 채택이므로 불변식 2는
사상되어 적용된다: provenance = referent의 home, 검증 = 사용 실재 grep. 정의 속 behavior
...
쓰기 전 기존 home을 grep해 있으면 그 자리에서 갱신한다. 태그 시스템은 도입하지 않는다.
...
선행한다. 상충 검출의 3단 분업: 쓰기 시점(기존 home grep) / 병합 시점(post-rebase 스캔) /
주기(refine:docs).
...
**동거**(정본과 운반층 원본이 이 레포에 있어 같은 tx로 개정되고, 이 레포의 refine:docs가 주기
대조) / **방향**(배포본 머리에 정본 좌표 1행 — `https://github.com/clomia/claude-automata/blob/main/MEMORY.md` — 충돌 시
정본이 이긴다. 배포본은 타 레포에서 읽히므로 좌표는 자기완결 URL이어야 한다) / **버전 경계**(배포는 플러그인 릴리스 단위, 릴리스 감사가 정합을 점검) /
...
- 병렬 close의 분 단위 경합 창 — post-rebase 스캔 이후·병합 이전에 상대가 병합하는 창은
  클라이언트측에서 제거 불가다. 씨앗의 ruleset은 up-to-date 강제를 포함하므로 **시도가 성공한
  레포에서는 재rebase→재스캔이 강제되어 창이 닫힌다** — 단 tx는 서버측을 보증하지 못한다
  (실패는 1행 고지 후 진행), 성공해도 admin 토큰은 관문을 우회·철거할 수 있다. 서버측이 주는
  것은 불변 보증이 아니라 **우회의 감사 가능화**다. `gh pr checks`는 required 지정과 무관하게
  보고된 전 체크를 감시하므로 close 경로의 게이트는 서버 설정 없이도 성립한다 — 기여 주체는
  전부 클로드 코드 에이전트라는 설계 전제(ARCHITECTURE) 아래 tx 밖 병합 경로는 고려 대상이
  아니다.
- 훅은 로컬 git 경로만 지배한다 — 에이전트가 원격 쓰기 도구(gh api·MCP push 류)나 push
  refspec으로 base에 직접 쓰는 레인은 클라이언트가 막지 못한다. 서버측 보호가 성립한
  레포에서만 닫힌다.
