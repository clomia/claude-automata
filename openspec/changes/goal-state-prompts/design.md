...
- **엔진이 가르치는 것은 스킬이 재언하지 않는다.** blocked 상태의 instructions apply는
  ...
  시 에러가 가르치고, 체크박스 규칙은 apply payload가 나르며, design 포함 조건은 엔진의
  design instructions가 그 소비 시점에 직접 나른다("When to include design.md"). 단 게이트 실패 시 엔진의
  오라우팅(존재하지 않는 업스트림 스킬 권고)은 apply의 tx:plan 복귀 문장이 상시 오버라이드다
  — 삭제 금지 기록.
- CI 부재의 재시도 명령형은 삭제하고 사실형("PR 직후의 부재는 스케줄링 지연일 수 있다")만
  남긴다 — 체크 부재의 두 독해가 모두 차단인 시점에 시간만이 구분자이므로, 사실이 재시도를
  재구성한다(실측 합의).
