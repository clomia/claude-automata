# ploop-wake-integrity

An armed loop sleeps only when something is guaranteed to wake it. Two surfaces enforce
this: a launch-time gate keeps background waits mortal, and the stop gate refuses to
sleep silently into a state where no gated shell can ever exit.
