...
**Mission은 명백한 목표다.** 요구사항을 받아서 처리하고, 목표를 모두 달성하면 끝난다.
루프는 이 목표를 향해 수렴한다 — advisor는 목표가 모두 커버되면 종료를 판단한다.
...
2. 수집한 정보를 판단 기준으로 쓸 수 있는 축으로 종합하고, Anchor file save path에 마크다운 형식으로 mission을 작성하라.
...
- [IMPORTANT] **허용 범위**와 **달성할 목표**가 명확히 정의되어야 한다.
...
- 구조는 Background → Mission → Constraint → Reference 순이다. `# Mission`(달성할 목표)과 `# Constraint`(허용 범위)는 필수, `# Background`(목표의 배경)와 `# Reference`(관련 링크 — URL·파일 경로)는 선택이며, 섹션은 자유롭게 추가할 수 있다.
