...
- [IMPORTANT] 백그라운드 작업(shell·agent·workflow·monitor 등)이 남아 있는 동안 턴을 끝내지 마세요.
  - advisor 호출 지시는 포그라운드에 아무것도 남지 않을 때 발행됩니다. 작업이 남은 채 턴을 끝내면 미완의 라운드가 심사됩니다.
  - 가장 먼저 끝나는 작업이 돌아오는 포그라운드 대기를 반복하세요.
    (예: 1시간·3시간짜리 두 Job이 있다면 둘 중 하나가 끝나면 돌아오는 대기)
  - 적절한 포그라운드 대기 수단이 없다면 `AskUserQuestion`으로 사용자에게 알리세요.
