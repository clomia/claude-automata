...
main ─ Agent(advisor) ───────────> advisor (depth 1)
   ...
   |                                  └ Write region / termination token to advice.md
...
3. **loop 상태는 hook이 단독 소유.** advisor는 region(또는 종료 토큰)을 `advice.md`에 Write만 하고,
   hook이 그 파일을 읽어 round·regions·done을 모두 기록한다. `advice.md`가 region/종료의 **유일 채널**이라
...
9. **자발 advisor 호출 차단(PreToolUse 게이팅).** main이 hook 지시 없이 스스로 advisor를 부르면
   ...
   `advice.md`를 엉뚱한 시점에 덮어써 region 채널을 오염시킨다. Stop이 호출을 지시할 때만 1회용 토큰을
   ...
   소비되지 않고 남는다 — Stop은 그 라운드의 region 기록을 건너뛰어 직전 region이 중복 기록되지 않게 한다.)
10. **advisor·narrator 호출은 동기다(`run_in_background=false`).** Agent 툴은 이 빌드에서 기본 async라,
    백그라운드 호출은 region을 남기지 않고 launch acknowledgement만 돌려준다. main은 **foreground**이고,
