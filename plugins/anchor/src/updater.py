"""Update notifier — SessionStart hook that surfaces newer anchor releases.
...
from the per-session {session_id}_* files used by the other anchor hooks.
"""
...
REMOTE_URL = "https://raw.githubusercontent.com/clomia/claude-automata/main/plugins/anchor/.claude-plugin/plugin.json"
PLUGIN_NAME = "anchor"
...
def check_for_update() -> None:
    ...
    if cached_remote and is_newer(cached_remote, local_version):
        message = (
            f"anchor update available: {local_version} -> {cached_remote}\n"
            f"Run: claude plugin marketplace update claude-automata "
            f"&& claude plugin update anchor@claude-automata"
        )
