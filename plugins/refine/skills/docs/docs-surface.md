...
고정 이름 — 구조 단위는 `ARCHITECTURE.md`(storefront 의무 단위는 README 겸용), 횡단 domain은 domain 이름 — 이름이 회상 키다. hub는 위성에 link 위임하고 어느 방향도 재서술하지 않는다. 배제 기록은
...
**B 매 턴 규약**(전부 충족): 매 session 참 / 위반이 회상보다 먼저 온다 / 기계 강제 불가 /
