...
parallax-loop finds the regions Claude overlooked, round after round, and keeps
...
- Define the mission with Claude, then hand it off with `/parallax-loop:run`.
...
- Check progress with `/parallax-loop:log`.
...
parallax-loop reimplements the same parallax mechanism — an isolated advisor
...
main session's quota, so parallax-loop runs **within subscription terms**. parallax
...
parallax-loop's durability hook runs via **uv**. Install it from
...
claude plugin install parallax-loop@claude-automata
```
...
claude plugin update parallax-loop@claude-automata
```
