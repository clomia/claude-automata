...
- **목적을 가지고** — 목적이란 종료 조건이 없으며 나아가야 하는 방향을 뜻한다. AI Automata는 스스로 소유자로부터 목적을 구성한다.
- **AI를 영속적으로 실행시키는** — 여기서 AI란 Foundation Model을 내포한 도구를 총칭한다. AI Automata는 컨텍스트 보존 메커니즘을 가지며 이를 통해 AI를 계속 실행시킨다.
- **재귀적 자기개선** — AI Automata는 AI가 목적을 위해 스스로를 계속 개선하도록 유도한다. 개선 동작 자체를 포함한 모든 구성 요소가 개선 대상이다.
