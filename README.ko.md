...
- `/refine:code [영역]` — 코드 아키텍처 최적화. 안티패턴을 합의로 걸러내고 최고 ROI 리팩토링만 적용합니다.
...
- 트랜잭션은 **무결성 경계**입니다 — open부터 close까지가 하나로 묶이고, 무결성이 검증되어야만 닫힙니다.
- 전 과정이 tx 내장 스킬로 돕니다: `/tx:open`이 base 브랜치에서 `tx-*` 분기를 열고 씨앗을 심으며, `tx:plan`·`tx:apply`·`tx:verify`가 변경을 이끌고, `/tx:close`가 docs 게이트와 CI 통과 후 base로 squash merge합니다. 그 사이의 base 브랜치는 가드 훅들이 지킵니다.
