...
**4단계 — 요약**: state/session-summary.md에 기록하라:
  취한 접근법과 이유, 불확실했던 결정, 기각한 대안과 이유, 타협한 부분

이 session-summary.md는 Stop Hook Agent(인지 부하 트리거)의 핵심 입력이다.
Stop Hook Agent는 이 요약과 자신의 독립적 사고를 대조하여 미탐색 방향을 식별한다.
상세: [cognitive-load-trigger.md](components/cognitive-load-trigger.md) §3.
```
...
| **Q-3** 인지 부하 최대화 | 자기 주도(4단계 프로토콜) + 외부 주입(Stop hook agent 독립 사고 → 미탐색 방향). [상세](components/cognitive-load-trigger.md) |
| **Q-4** 트리거 5원칙 | Q-4a~Q-4e 준수. session-summary.md(구조화된 자기 평가) + 독립 사고 → 별도 컨텍스트 agent → 추상적 방향 주입 |
